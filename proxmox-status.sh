...
process_cluster() {
    ...
    while read -r NODE; do
        ...
        NODE_INFO=$(jq -c --arg raw "$NODE" --arg short "$SHORT" '
            def norm($s): ($s // "" | ascii_downcase | split(".")[0]);
            first(
              .data[]?
              | select((.node // "") as $n
                  | (norm($n) == norm($raw))
                    or (norm($n) == norm($short)))
            ) // {}
        ' "$TMP_PREFIX.nodes.api" 2>/dev/null || echo '{}')

        API_NODE=$(jq -r '.node // empty' <<< "$NODE_INFO")
        [[ -z "$API_NODE" ]] && API_NODE="$SHORT"

        # Determine real node status from node endpoint first, because /nodes
        # listing may not reliably map configured FQDNs to API node IDs.
        if pve_get "nodes/$API_NODE/status" "$TMP_PREFIX.met"; then
            STATUS=$(jq -r '.data.status // "online"' "$TMP_PREFIX.met" 2>/dev/null || echo "online")
        else
            STATUS=$(jq -r '.status // "offline"' <<< "$NODE_INFO")
            echo '{"data":{}}' > "$TMP_PREFIX.met"
        fi
